...
- 修改前必须阅读本文件和相关目录。
- 新行为必须先写对应 Go 单元测试，再写实现；纯文档和机械配置除外。
...
- 代码层必须有对应 Go 单元测试，文件名必须是 `*_test.go`。
- `docs/tests/` 只记录测试计划和执行结果，不能替代 Go 单元测试。
- Go 单元测试必须覆盖核心成功路径和关键失败路径。
- 文档层必须在 `docs/tests/` 记录测试计划、测试命令、执行结果和未覆盖风险。
- 提交前必须执行完整验证命令。
- 除非用户明确批准临时提交，否则全部必需测试必须通过后才能提交。
- 如果环境缺少 Go、Docker 或网络权限，不能声称通过；必须记录阻塞原因，并等待补齐环境或用户批准例外。
...
如果当前机器缺少 `go`、Docker 或网络权限，最终回复必须明确说明哪些验证没有执行；功能代码提交默认不能绕过必需测试。
